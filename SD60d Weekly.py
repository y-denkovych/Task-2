...
class Value(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.Schedule.On(self.DateRules.Every((DayOfWeek.Monday)),self.TimeRules.AfterMarketOpen(self.symbol), self.Selection)
        ...
        for symbol in self.long:
            if symbol in data and data[symbol]:
                ...
                if symbol in self.Portfolio and self.Portfolio[symbol].Quantity != 0:
                    ...
                    close = self.History(symbol, 60, Resolution.Hour)["close"]
                    standdev = np.std(close)
                    current_price = data[symbol].Close
                    stop_price_long = current_price - standdev
        ...
        for symbol in self.short:
            if symbol in data and data[symbol]:
                ...
                if symbol in self.Portfolio and self.Portfolio[symbol].Quantity != 0:
                    ...
                    standdev = np.std(close)
                    current_price = data[symbol].Close
                    stop_price_short = current_price + standdev
